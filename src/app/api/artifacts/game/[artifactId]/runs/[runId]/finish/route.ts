...
import { finishGameRun } from "@/features/artifacts/games/run-service";
import { getCurrentActor } from "@/features/identity/current";

const querySchema = z.object({ workspaceId: z.string().uuid() }).strict();
const bodySchema = z.object({ score: z.number().int().min(0) }).strict();
...
export async function POST(
  ...
  try {
    ...
    return Response.json(
      await finishGameRun(actor, { artifactId, runId, ...body.data, ...query.data }),
    );
  } catch (error) {
