...
> 注意：若本文件与 `00` 到 `08` 号源文件不一致，应以源文件为准并重新合并生成。
...
`Spectra` 以统一工作台形式组织核心能力，覆盖以下主要环节：
...
由此，系统形成了以教师生成、修改、导出与资产沉淀为主的核心业务闭环，并为后续分享与复用场景提供支撑。
...
可以说，智能生成层解决“生成内容”，库编排层解决“让内容变成长期可复用的课程资产”。二者联动后，课程资料库既是沉淀结果，也是多模态内容持续外化的统一来源。教师维护权威课程空间，学生默认只读访问；需要持续沉淀个人资料时，再创建自己的学习空间。多人协作者则通过独立会话提交候选变更，由空间维护者审核合入。

### 4.4.6 数据存储层

系统主要使用：

- `SQLite`：存储项目、用户、会话、任务、权限、引用关系、版本和资源元数据
...
1. 在当前验证中，备课过程中的重复整理与切换成本有所下降。
2. 多模态资料得到更统一的利用，已有素材的复用效率有所改善。
...
综合功能测试、流程测试、工程测试和应用效果评估，当前结果表明 `Spectra` 已不只是单点生成工具，而是围绕教学主链形成了较完整的生成、修改、导出与资产沉淀能力。

更重要的是，项目通过“无感化资产沉淀”将教师原本就要完成的课件生产过程，转化为课程资料库建设过程。结合轻量版本、导出物来源记录和引用复用机制后，`Spectra` 已具备从内容生成进一步走向内容组织与状态管理的能力，也为服务外包项目、教育数字化平台和教学资源管理场景提供了较清晰的应用方向。
...
- 支持知识网络复用：可基于标准课程库派生个人课程库，复用主骨架并叠加个性化补充内容。
...
- 支持个性化的二次学习与内容生成。
- 在授权访问或按需生成时，可获取基于当前课程状态的内容，而不是依赖一次性过期文件。

从商业角度看，学生并不只是被动使用者。由于学生可以围绕主数据库生成个人导图、摘要、练习和复习资产，高级生成能力、扩展存储空间和更深层个性化服务也构成了后续增值探索的方向，这使学生既是用户，也是潜在的扩展服务对象。
...
这一模式使 `Spectra` 具备从工具交付进一步延展到平台化交付的可能性，也为更高的客户粘性与续费能力提供了空间。
...
4. 增值服务费：针对高级模板、课程资料库扩展能力、学生端导图生成、个人资产空间、高级学习生成能力与高级引用复用能力等功能提供增值收费。
...
3. 对学生而言，它提供了进一步获取结构化课程内容与个性化学习材料的可能。
...
3. 校级部署：以课程资料库、权限控制、引用复用和校本知识资产管理切入。
4. 平台扩张：以学生端访问、个性化学习材料生成和跨库知识网络切入。
...
`Spectra` 的竞争壁垒不只是“能生成 PPT”，而是以下几层：

1. 教学场景壁垒：围绕教师备课这一高频刚需场景深度设计。
2. 数据资产壁垒：课程内容在使用过程中自然沉淀为资料库，形成长期累积价值。
3. 网络演化壁垒：库之间可形成可控引用与协作关系，提升复用深度和迁移成本。
4. 治理壁垒：正式版本、候选变更审核与权限拆分使系统更适合组织级使用。
5. 双边网络壁垒：教师持续生产内容，学生持续消费和二次生成内容，使平台形成双边增长结构。
...
同样重要的是，课程资料库在商业上承担了统一生成源的角色。它使课件、教案、导图、动画、互动内容和学生侧学习材料不再彼此割裂，而是围绕同一课程知识结构持续演化。这种一致性直接转化为更高的资源复用效率、更强的组织治理能力和更稳固的平台壁垒。进一步地，以项目空间、引用关系、候选变更审核和学习空间为核心的演进路径，使 `Spectra` 具备从单一教学工具走向内容协作平台的可能性；而库之间形成的主基底加辅助引用网络，则为校本课程复用和组织级课程治理提供了更直接的商业抓手。
...
本章用于将比赛原始题目 `docs/project/requirements.md` 与 `Spectra` 方案逐项对应，确保提交材料不仅叙事完整，也完整满足题目要求。对于 `Spectra` 而言，题目要求是系统必须满足的基础能力，而不是另外附加的一组勾选项。系统的核心解法主线是：以教师教学思路为起点，将原本就要发生的课件生产过程，转化为“理解、生成、优化、沉淀、分发”一体化工作流。课程资料库并不是额外附加物，而是这一工作流对题目要求的统一抽象结果。进一步说，系统真正的核心对象是“库”，题目要求中的 PPT、教案、导图和网页等内容都只是库的外化结果。
...
`Spectra` 对这五项要求形成了明确回应，并进一步以“无感化资产沉淀 + 课程资料库分享”将原题中的生成、优化、导出和分发能力整合为一条更完整的业务主线。题目要求中的核心功能，在 `Spectra` 中都能找到对应实现或设计支撑；在此基础上，方案进一步给出了以库、引用、版本和权限为骨架的系统化组织方式。
...
| 沉淀长期可复用成果 | 系统将正式生成结果沉淀为库状态，并以版本标识稳定状态 |
...
| 原题要求 | `Spectra` 覆盖方式 |
|---|---|
| 生成结构完整、内容丰富的多模态课件初稿 | 系统先生成结构化内容，再按需导出多模态课程成果 |
...
| 形成互动-生成-反馈-再生成闭环 | 系统建立版本化生成闭环，并在正式入库后形成新的稳定版本 |
...
| 实用性和创新性 | 以教师备课真实场景为主线，并通过无感化资产沉淀、库化管理和引用复用形成更高层次的解法统一性 |
...
`教师表达教学思路 -> 智能体主动澄清 -> 多模态资料与本地知识库融合 -> 生成课程成果 -> 预览反馈再生成 -> 无感沉淀为课程资料库 -> 形成版本化库状态 -> 按权限向学生分发并支持二次利用`

这条主线既完整满足原题每一项硬性要求，又使方案在业务上更完整、在工程上更闭环、在商业上更具延展性。

更重要的是，`Spectra` 认为多模态的关键不在于“有多少种模态”，而在于“是否存在一个统一的课程数据库作为生成源”。只有当 PPT、教案、导图、动画、网页和互动内容都来自同一课程知识结构时，题目要求中的多模态解析、多模态生成、迭代优化和资源分发才会真正连成一个完整系统。数据库由教师作为权威源持续更新，学生默认先作为访问者读取与按需生成；只有在创建自己的学习空间后才持续沉淀个人资产，这使系统既保持权威性，也具备双边扩展能力。进一步地，库之间还可以通过主基底引用与辅助引用形成可控的课程知识网络，让题目要求中的“生成”能力进一步演化为可复用、可协作、可持续更新的组织级能力。
