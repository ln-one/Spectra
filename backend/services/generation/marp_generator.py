...
async def call_marp_cli(
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            logger.error(f"Marp CLI failed with return code {process.returncode}: {error_msg}")
            raise ToolExecutionError("Marp CLI", error_msg, process.returncode)
        
        logger.debug(f"Marp CLI executed successfully")
...
async def generate_pptx(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"[Task: {task_id}] Markdown written to {markdown_file} ({len(full_markdown)} bytes)")
