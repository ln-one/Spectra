...
import { ArrowLeft, Play, Plus, RefreshCw, Sparkles, Trash2 } from "lucide-react";
...
export function CompactOutlineView({
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-col gap-3 h-full">
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={onBack} className="h-8 text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100">
            <ArrowLeft className="w-3.5 h-3.5 mr-1" />
            返回
          </Button>
          <span className="text-xs font-medium text-zinc-500 truncate max-w-[120px]">{topic}</span>
        </div>
        <Button variant="ghost" size="sm" className="h-8 text-xs text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100">
      ...
      <motion.div variants={itemVariants} className="flex-1 min-h-0">
        <ScrollArea className="h-full pr-2" ref={scrollAreaRef as React.RefObject<HTMLDivElement>}>
              ...
              {slides.map((slide, index) => (
                ...
                >
                  <div className="flex items-start gap-2">
                    <span className="text-xs font-bold text-zinc-400 shrink-0 mt-0.5 w-4">{String(index + 1).padStart(2, "0")}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-zinc-700 truncate">{slide.title}</p>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {slide.keyPoints.slice(0, 2).map((point, i) => (
                          <span key={i} className="text-[10px] text-zinc-400 bg-zinc-100 px-1.5 py-0.5 rounded">
                            {point}
                          </span>
                        ))}
                        {slide.keyPoints.length > 2 ? (
                          <span className="text-[10px] text-zinc-400">+{slide.keyPoints.length - 2}</span>
                        ) : null}
                      </div>
                    </div>
                    <Button
      ...
      <motion.div variants={itemVariants} className="space-y-2 pt-2 border-t border-zinc-200/60">
        {isOutlineHydrating ? <p className="text-[11px] text-zinc-500 text-center">大纲加载中，请稍候...</p> : null}
        {generationFailed ? <p className="text-[11px] text-red-500 text-center">{generationFailed}</p> : null}
        {outlineIncomplete ? (
          <p className="text-[11px] text-zinc-500 text-center">大纲生成中：{slides.length}/{expectedPages} 页</p>
        ) : null}
        ...
        <AnimatePresence mode="wait">
          {!isGenerating ? (
            <motion.div key="start-button" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <Button
              ...
              <p className="text-[10px] text-zinc-400 text-center mt-1.5">预计 {totalEstimatedMinutes} 分钟 · 约 {estimatedTokens} tokens</p>
            </motion.div>
          ) : (
            <motion.div key="progress-section" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="space-y-2">
              ...
              <Button onClick={onGoToPreview} className="w-full h-9 border border-zinc-800 bg-zinc-900 text-zinc-50 text-xs font-medium shadow-sm hover:bg-zinc-800">
