...
function inferVisualType(text: string): AnimationSpecPreview["visualType"] {
  if (
    /分层|层次结构|网络层|结构|组成|部件|层级|拆解|剖面/.test(text)
  ) {
...
export function buildAnimationSpecPreview(params: {
  ...
  const objects = visualType === "structure_breakdown" ? extractObjects(topic, focus) : [];
  ...
  if (visualType === "relationship_change") {
    return {
      ...
      objectDetails: [],
      scenes: [
        { title: "先看变化对象", description: `先明确 ${topic} 中哪些量在变化。` },
        { title: "突出关键拐点", description: `强调转折、增减和因果关联。${rhythmHint}` },
  ...
  return {
    ...
    scenes: [
      ...
      { title: "展开关键过程", description: `按步骤突出阶段切换和关键因果。${rhythmHint}` },
