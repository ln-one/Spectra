...
# 第 4 章 已说明系统架构形态、分层逻辑与主链设计。本章旨在详述系统底座的实现细节：
...
1.  教师通过 Studio 实现可管理、可修改、可回流的多模态内容工坊；

2.  预览、渲染与标准交付的统一结果链；

3.  多模态资料进入、检索增强与证据组织的稳定底座；

4.  身份、组织与成员边界的独立管理，支撑系统长期产品化。

本章说明：成熟模型、检索方法、远端服务、标准导出链和正式状态能力，已通过标准化通信协议与数据交换契约组织成可控、可解释、可评估的系统。

<!-- figure-block {"figure_id": "fig-image9-01", "source_image": "image9.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-1 技术承接链。", "width_hint": "6.86319in", "height_hint": "8.73611in"} -->
![](../assets/media/image9.png){width="6.86319in" height="8.73611in"}
...
<!-- figure-block {"figure_id": "fig-image10-02", "source_image": "image10.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-2 对应外化结果回到正式知识状态的路径。", "width_hint": "6.62992in", "height_hint": "7.47244in"} -->
![](../assets/media/image10.png){width="6.62992in" height="7.47244in"}
...
1.  Spectra Backend 建立 Session、组织上下文与任务状态；

2.  Diego 先形成课件大纲；

3.  教师确认 outline 后进入正式生成；

4.  生成结果进入 Studio 的真实 preview/refine 链；

5.  结果经由交付层和知识状态层完成绑定与沉淀。

<!-- figure-block {"figure_id": "fig-image11-03", "source_image": "image11.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-3 对应可确认、可修改、可回流的会话闭环。", "width_hint": "6.63333in", "height_hint": "8.73056in"} -->
![](../assets/media/image11.png){width="6.63333in" height="8.73056in"}

图 5-3 对应可确认、可修改、可回流的会话闭环。
<!-- /figure-block -->

::: {custom-style="Body Text"}
<!-- figure-block {"figure_id": "fig-image12-07", "source_image": "image12.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-4 对应由同一课程知识空间、同一会话主链和同一结果契约共同驱动的卡片执行体系。", "width_hint": "6.2868055555555555in", "height_hint": "8.134027777777778in"} -->
![](../assets/media/image12.png){width="6.2868055555555555in" height="8.134027777777778in"}
...
::: {custom-style="Body Text"}
<!-- figure-block {"figure_id": "fig-image13-08", "source_image": "image13.png", "source_chapter": "关键技术与实现", "source_caption": "这条原则已经在当前前端产品面中落实到非 PPT 产物上：当后端尚未返回真实内容时，前端只展示等待态。只有当正式产物返回后，系统才允许用户执行 preview、refine、history 和 download。", "width_hint": "7.730555555555555in", "height_hint": "3.2979166666666666in"} -->
![](../assets/media/image13.png){width="7.730555555555555in" height="3.2979166666666666in"}
...
1.  控制平面契约：Spectra Backend 负责 run / event / command / query 编排和结果整形；

2.  权责边界契约：Diego / Pagevra / Ourograph / Stratumind / Limora 各自承接正式能力结果；

<!-- figure-block {"figure_id": "fig-image14-09", "source_image": "image14.png", "source_chapter": "关键技术与实现", "source_caption": "回流契约：生成结果经 preview / export / bind / candidate change / version update 回到课程知识空间。", "width_hint": "7.951388888888889in", "height_hint": "3.1256944444444446in"} -->
3.  ![](../assets/media/image14.png){width="7.951388888888889in" height="3.1256944444444446in"}

回流契约：生成结果经 preview / export / bind / candidate change / version update 回到课程知识空间。
<!-- /figure-block -->
...
<!-- figure-block {"figure_id": "fig-image15-10", "source_image": "image15.png", "source_chapter": "关键技术与实现", "source_caption": "采用混合检索策略（dense + sparse）提升召回率与精确度。", "width_hint": "8.275in", "height_hint": "3.75in"} -->
  - ![](../assets/media/image15.png){width="8.275in" height="3.75in"}
...
1.  Dualweave 组织上传编排和远端解析入口，通过异步队列实现高并发处理；

2.  资料转化为可检索的文本、chunk、元数据与来源信息；

3.  Stratumind 进行查询改写、自适应规划、混合检索、可选重排、证据组织和结果打包；

4.  证据结果进入生成主链，支撑 Studio 内容工坊。

<!-- figure-block {"figure_id": "fig-image16-04", "source_image": "image16.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-8 对应从解析到底层证据组织的分层能力链。", "width_hint": "6.25in", "height_hint": "8.70903in"} -->
![](../assets/media/image16.png){width="6.25in" height="8.70903in"}

图 5-8 对应从解析到底层证据组织的分层能力链。
<!-- /figure-block -->
...
<!-- figure-block {"figure_id": "fig-image17-05", "source_image": "image17.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-9 对应身份与组织语义与课程知识空间本体的分离。", "width_hint": "6.09449in", "height_hint": "8.35039in"} -->
![](../assets/media/image17.png){width="6.09449in" height="8.35039in"}

图 5-9 对应身份与组织语义与课程知识空间本体的分离。
<!-- /figure-block -->
...
<!-- figure-block {"figure_id": "fig-image18-06", "source_image": "image18.png", "source_chapter": "关键技术与实现", "source_caption": "图 5-10 展示六个正式能力层如何围绕同一课程知识空间形成闭环。", "width_hint": "6.64097in", "height_hint": "8.75833in"} -->
![](../assets/media/image18.png){width="6.64097in" height="8.75833in"}

图 5-10 展示六个正式能力层如何围绕同一课程知识空间形成闭环。
<!-- /figure-block -->
