    ...
    StudioCardCapability(
        ...
        config_fields=[
            StudioCardConfigField(
                ...
                type=StudioCardFieldType.TEXT,
                required=True,
                placeholder="例如：光合作用电子传递流程",
            ),
            StudioCardConfigField(
                key="motion_brief",
                label="表现重点",
                type=StudioCardFieldType.TEXT,
                placeholder="例如：突出关键步骤切换与因果关系",
            ),
            StudioCardConfigField(
            ...
            StudioCardConfigField(
                ...
                type=StudioCardFieldType.SELECT,
                options=[
                    StudioCardConfigOption(value="slow", label="慢速讲解"),
                    StudioCardConfigOption(value="balanced", label="均衡"),
                    StudioCardConfigOption(value="fast", label="快速演示"),
                ],
                default_value="balanced",
            ),
        ],
        actions=[
            StudioCardAction(type="generate", label="生成 GIF 动画"),
            StudioCardAction(type="chat_refine", label="调整时长 / 节奏 / 重点"),
        ],
        notes="第一阶段动画正式产物固定为 GIF；生成后可推荐 1 页 PPT 供用户确认插入，但动画本体始终保持独立 artifact。",
    ),
    StudioCardCapability(
        id="speaker_notes",
        title="说课助手",
