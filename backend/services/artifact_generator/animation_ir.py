...
class SceneMeta(BaseModel):
    ...
    background_gradient: list[str] = Field(
        default=["#0a0e27", "#1a1e3a"], description="背景渐变色（两个十六进制颜色）"
    )
    ...
    def normalize_position(cls, v):
        ...
        if isinstance(v, str):
            ...
            if isinstance(v[0], str) or isinstance(v[1], str):
                ...
                y = {"top": 2, "bottom": -2, "middle": 0, "center": 0}.get(
                    str(v[1]).lower(), 0
                )
                return [x, y]
        return v
...
class AnimationAction(BaseModel):
    ...
    target: str | list[str] = Field(description="目标对象 ID（单个或多个）")
    params: dict[str, Any] = Field(
        default_factory=dict, description="动画参数，如 {run_time: 0.5, shift: [1, 0]}"
    )
    lag_ratio: float | None = Field(
        default=None, description="如果是多对象，LaggedStart 的 lag_ratio"
    )
...
class SceneTransition(BaseModel):
    ...
    type: Literal["fade_out_all", "clear", "replace"] = Field(description="转场类型")
    targets: list[str] | None = Field(
        default=None, description="如果是 replace，指定要替换的对象 ID"
    )
...
class TextBlock(BaseModel):
    ...
    offset: list[float] = Field(default=[0, 0], description="相对位置的偏移 [x, y]")
...
class AnimationPlan(BaseModel):
    ...
    transitions: list[SceneTransition] = Field(
        default_factory=list, description="场景转场（可选）"
    )
    text_blocks: list[TextBlock] = Field(
        default_factory=list, description="独立文本块（可选）"
    )
