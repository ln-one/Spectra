...
class TestFullGenerationFlow:
    ...
    async def test_generate_both_pptx_and_docx(
        ...
        # 生成 PPTX
        pptx_path = await service.generate_pptx(sample_courseware_content, task_id)

        # 生成 DOCX
        docx_path = await service.generate_docx(sample_courseware_content, task_id)
    ...
    async def test_generate_multiple_tasks(
        ...
        for i in range(3):
            pptx_path = await service.generate_pptx(
                simple_courseware_content, f"integration-test-multi-{i}"
            )
...
class TestPerformance:
    ...
    async def test_simple_pptx_generation_time(
        ...
        start = time.time()
        await service.generate_pptx(simple_courseware_content, "integration-test-perf")
    ...
    async def test_complex_pptx_generation_time(
        ...
        await service.generate_pptx(
            sample_courseware_content, "integration-test-perf-complex"
        )
        duration = time.time() - start
...
class TestErrorScenarios:
    ...
    @pytest.mark.asyncio
    async def test_invalid_markdown_handling(self, integration_output_dir):
        """测试处理无效 Markdown"""
        service = GenerationService(output_dir=str(integration_output_dir))
