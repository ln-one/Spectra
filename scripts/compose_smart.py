...
DEFAULT_OVERRIDE_FILE = ROOT / "docker-compose.override.yml"
...
def _extract_override_sections(path: Path) -> tuple[list[str], list[str]]:
...
def write_default_override(modes: list[tuple[ServiceSource, str]]) -> None:
    service_lines: list[str] = []
    volume_lines: list[str] = []

    for source, mode in modes:
        if mode != "local-source" or not source.override_file:
            continue
        override_path = ROOT / source.override_file
        if not override_path.exists():
            raise ValueError(
                f"Expected compose override file for {source.display_name}: {override_path}"
            )
        extracted_services, extracted_volumes = _extract_override_sections(
            override_path
        )
        service_lines.extend(extracted_services)
        volume_lines.extend(extracted_volumes)

    if not service_lines and not volume_lines:
        if DEFAULT_OVERRIDE_FILE.exists():
            DEFAULT_OVERRIDE_FILE.unlink()
        return

    lines = [
        "# Generated by scripts/compose_smart.py sync",
        "# Lets plain `docker compose up` follow the current local-source plan.",
    ]
    if service_lines:
    ...
    DEFAULT_OVERRIDE_FILE.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def sync_needed(channel: str, refs: dict[str, str]) -> str | None:
    env_values = load_env_lock()
    if not env_values:
        return f"missing {ENV_LOCK_FILE}"
    ...
    if mismatches:
        return (
            f"{ENV_LOCK_FILE} does not match the current stack lock: "
            + "; ".join(mismatches)
        )
...
def ensure_runtime_sync(channel: str, refs: dict[str, str]) -> None:
    reason = sync_needed(channel, refs)
    if reason is None:
        return

    print(
        "[compose-smart] Auto-syncing compose environment before docker compose "
        f"({reason})."
    )
    handle_sync(channel)


def should_auto_build(argv: list[str], modes: list[tuple[ServiceSource, str]]) -> bool:
    if not argv or argv[0] != "up":
        return False
    if "--build" in argv:
        return False
    if any(arg.startswith("--build=") for arg in argv):
        return False
    return any(mode == "local-source" for _, mode in modes)
...
def handle_sync(channel: str) -> int:
    ...
    write_default_override(modes)
    print(f"[compose-smart] Wrote {ENV_LOCK_FILE}")
    if DEFAULT_OVERRIDE_FILE.exists():
        print(f"[compose-smart] Wrote {DEFAULT_OVERRIDE_FILE}")
    else:
        print(f"[compose-smart] Removed {DEFAULT_OVERRIDE_FILE} (lock-only compose)")
    return 0
...
def build_compose_command(
    *,
    ...
    modes = resolve_service_modes()
    refs, locks = ensure_sync_ready(channel, modes)
    ensure_runtime_sync(channel, refs)
    ...
    if should_auto_build(final_argv, modes):
        print(
            "[compose-smart] Local source detected; adding '--build' so code changes "
            "land without a separate rebuild step."
        )
        final_argv.insert(1, "--build")

    command.extend(final_argv)
