...
class TestGeneratePPTX:
    ...
    async def test_generate_pptx_success(
        ...
        with patch('services.generation._generate_pptx', new_callable=AsyncMock) as mock_gen:
    ...
    async def test_generate_pptx_with_custom_template(
        ...
        with patch('services.generation._generate_pptx', new_callable=AsyncMock) as mock_gen:
            ...
            await generation_service.generate_pptx(
                mock_content,
                "test-task",
                config
            )
    ...
    @pytest.mark.asyncio
    async def test_generate_pptx_timeout(
        self,
        generation_service,
        mock_content
    ):
...
class TestGenerateDOCX:
    ...
    async def test_generate_docx_success(
        ...
        with patch('services.generation._generate_docx', new_callable=AsyncMock) as mock_gen:
            mock_gen.return_value = expected_path
    ...
    @pytest.mark.asyncio
    async def test_generate_docx_with_template(
        self,
        generation_service,
        mock_content
    ):
        """测试使用模板生成 DOCX"""
        config = TemplateConfig()
        generation_service.template_service.get_pandoc_template_path.return_value = "template.docx"
        
        with patch('services.generation._generate_docx', new_callable=AsyncMock) as mock_gen:
            mock_gen.return_value = "test.docx"
            
            await generation_service.generate_docx(
                mock_content,
                "test-task",
                config
            )
...
class TestFilePathSafety:
    ...
    @pytest.mark.asyncio
    async def test_task_id_sanitization(
        self,
        generation_service,
        mock_content
    ):
...
class TestConcurrentGeneration:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_pptx_generation(
        self,
        generation_service,
        mock_content
    ):
