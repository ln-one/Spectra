...
_VALID_SUBJECT_FAMILIES = {
    "generic_process", "protocol_exchange", "pipeline_sequence",
    "lifecycle_cycle", "traversal_path", "energy_transfer",
    "structure_layers", "trend_change",
}
_VALID_SHOT_TYPES = {"intro", "focus", "summary"}
_VALID_TRANSITIONS = {"cut", "dissolve", "soft_wipe", "blinds", "shutter", "fade", "slide", "zoom", "wipe"}
_VALID_CAMERAS = {"wide", "medium", "close", "track_left", "track_right", "zoom_in", "zoom_out"}
_VALID_KINDS = {
    "endpoint", "payload", "step", "layer", "node", "stage",
    "point", "source", "channel", "converter", "output",
}
...
def _build_prompt(content: dict[str, Any], rag_snippets: list[str]) -> tuple[str, str]:
    ...
    teaching_goal = str(
        content.get("teaching_goal") or content.get("focus") or content.get("summary") or topic
    ).strip()
    ...
    rag_text = "\n".join(f"- {s}" for s in rag_snippets[:3]) if rag_snippets else "（无参考资料）"
...
def _validate_and_repair(data: dict[str, Any], content: dict[str, Any]) -> dict[str, Any]:
    ...
    # focus
    focus = str(data.get("focus") or content.get("focus") or content.get("topic") or "").strip()
...
async def generate_animation_spec_with_llm(
    ...
    try:
        ...
        logger.warning("animation_spec_llm: failed with %s, caller should fall back", exc)
        return None
