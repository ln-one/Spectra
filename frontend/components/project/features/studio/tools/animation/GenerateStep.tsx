...
export function GenerateStep({
  ...
  const fallbackSpecPreview = buildAnimationSpecPreview({
    topic,
    focus,
    rhythm,
  });
  const specPreview = serverSpecPreview
    ? {
        ...
        objectDetails: Array.isArray(serverSpecPreview.object_details)
          ? (serverSpecPreview.object_details as Array<{
              label: string;
              role: string;
            }>)
        ...
        <div className="space-y-2 sm:col-span-2">
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] text-emerald-900">
            ...
            recommendedDurationSeconds !== null ? (
              <p className="mt-1">
                当前规格预计分镜 {sceneCount} 个，最低可用{" "}
                {Math.round(minimumDurationSeconds)} 秒，舒适观看推荐{" "}
                {Math.round(recommendedDurationSeconds)} 秒。
              </p>
            ) : (
              <p className="mt-1">
