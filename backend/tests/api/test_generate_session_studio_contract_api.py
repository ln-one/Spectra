...
def test_get_studio_card_sources_prioritizes_current_head(app, _as_user):
    client = TestClient(app)
    current_by_lineage = SimpleNamespace(
        id="a-ppt-011",
        type="pptx",
        metadata={
            "title": "当前课件",
    ...
    superseded_artifact = SimpleNamespace(
        ...
        metadata={
            "title": "旧课件",
            "is_current": False,
            "superseded_by_artifact_id": "a-ppt-011",
        },
        visibility="project-visible",
        basedOnVersionId="v-old",
        sessionId="s-009",
    ...
    explicit_current = SimpleNamespace(
        id="a-ppt-013",
        type="pptx",
        metadata={
            "title": "显式当前课件",
        ...
        visibility="project-visible",
        basedOnVersionId="v-explicit",
        sessionId="s-011",
        updatedAt=datetime(2026, 4, 9, 9, 0, tzinfo=timezone.utc),
    )
...
def test_mindmap_chat_refine_uses_formal_execution_result_path(app, _as_user):
    client = TestClient(app)
    execution_result = _ExecutionResult(
        {
            "resource_kind": "artifact",
    ...
    with patch(
        "routers.generate_sessions.studio_cards.execute_studio_card_refine_request",
        AsyncMock(return_value=execution_result),
    ) as refine_mock:
        response = client.post(
    ...
    assert response.status_code == 200
    request_body = refine_mock.await_args.kwargs["body"]
    assert request_body.refine_mode == RefineMode.STRUCTURED_REFINE
    assert request_body.config["chat_refine_scope"] == "full_map"
    assert (
        response.json()["data"]["execution_result"]["artifact"]["id"]
        == "a-mindmap-002"
    )
