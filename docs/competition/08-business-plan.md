...
```mermaid
flowchart LR
    A["创作者 / 团队"] --> B["Project / 课程库"]
    H["机构空间 / 组织治理"] --> B
    B --> C["Reference / 引用关系"]
    B --> D["Version / CandidateChange"]
    B --> E["多模态外化结果"]
    B --> F["授权 / 订阅 / 购买"]
    G["学习者 / 使用者"] --> F
    E --> G
...
```mermaid
flowchart LR
    A["创作者低门槛建库"] --> B["课程库被引用 / 使用 / 外化"]
    B --> C["订阅 / 增值 / 授权 / 平台抽成 / 机构交付"]
    C --> D["沉淀更多课程库 / 引用关系 / 学习行为"]
    D --> E["增强检索 / 生成 / 推荐 / 个性化学习"]
    E --> F["吸引更多创作者 / 学习者 / 机构"]
    F --> A
```
...
```mermaid
flowchart LR
    A["阶段一：教师 / 创作者入口"] --> B["阶段二：团队协作与课程资产流通"]
    B --> C["阶段三：机构私有化与组织治理"]
    C --> D["阶段四：AI 原生教育基础设施"]
```
