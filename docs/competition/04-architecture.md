...
图 4-1 对应系统总体分层结构，重点是把产品面、控制平面和正式能力层放到同一视野里。
...
图 4-2 进一步解释 `Studio` 为什么是系统级产品面，而不是功能清单。重点不在于“有八张卡”，而在于同一课程知识空间如何同时承接课堂主交付、互动练习和教学辅助三类外化结果。
...
真正需要被看见的，不只是六个正式能力层各自做什么，而是整套系统如何围绕不同 authority 共同成立。`Spectra` 自身并不是空壳，它负责工作台、控制平面和运行底座；六个正式能力层则分别承接生成、渲染、知识状态、资料进入、检索引证和身份治理。这些部分放在一起，才构成今天这套系统的真实工程形态。
...
这张矩阵说明的，是整套系统如何共同成立。前端工作台、控制平面和数据底座提供统一产品面与运行底座；六个正式能力层则围绕不同 authority 分别承担生成、渲染、知识状态、资料进入、检索引证和身份治理。生成、渲染、知识状态、资料进入、检索引证和身份治理并不是同一种工程问题，因此它们天然需要不同的运行时、框架组合和工程路径。`Spectra` 的工程分量，正体现在这种围绕 authority 分治形成的异构系统感上。

## 4.6 三条核心主链

第 4 章最重要的，是三条核心主链。系统之所以可信，是因为这三条链把资料、生成、交付、沉淀和演化接成了同一套业务语言。
...
如果从外部可信度角度再往前一步看，当前系统已经具备三类成熟信号。
...
第三，这套运行现实天然支持从演示、试点到组织级交付的渐进扩展。它已经具备持续协作、稳定交付和后续组织化部署基础。
