...
`逐页生成 slide markdown / 结构化内容 -> 逐页预览渲染 -> 全部完成后整份 Marp CLI 导出 pptx`

说明：

- “像 Gamma 一样逐页长出来”的核心是预览层渐进出现
- `Marp CLI` 主要负责最终正式导出
- 不建议把“每页都跑一次完整 CLI”作为主体验实现方式
- 截图或静态快照可以作为临时兜底，但不应继续被视为长期主路线
...
- 前端按 Marp 风格渲染 HTML 预览
- 每页内容到达后立即插入预览区
- 截图或静态图仅作为兼容、兜底或高保真回填方案
...
- PPT 预览区逐页出现
- 用户能看到当前已生成到哪里
- 预览区优先展示可交互的 Marp 风格 HTML 页面，而不是把截图作为唯一载体
...
- 预览层从“依赖截图兜底”提升为“优先逐页 HTML 预览，最终整份导出收口”
