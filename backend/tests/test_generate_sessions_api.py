...
@pytest.mark.anyio
async def test_create_session_uses_numbered_default_title(
    app, mock_db_service, _as_user
):
    mock_db_service.generationsession.find_first = AsyncMock(return_value=None)
    mock_db_service.generationsession.count = AsyncMock(return_value=2)
    ...
    assert response.status_code == 200
    create_data = mock_db_service.generationsession.create.await_args.kwargs["data"]
    assert create_data["displayTitle"] == "新建会话3"
    assert create_data["displayTitleSource"] == "default"
...
@pytest.mark.anyio
async def test_get_studio_card_execution_plan_returns_ppt_card_bindings(app, _as_user):
    client = TestClient(app)

    response = client.get("/api/v1/generate/studio-cards/courseware_ppt/execution-plan")

    assert response.status_code == 200
    plan = response.json()["data"]["execution_plan"]
    assert plan["card_id"] == "courseware_ppt"
    assert plan["initial_binding"]["transport"] == "session_create"
    assert plan["initial_binding"]["endpoint"] == "/api/v1/generate/sessions"
    assert "template" in plan["initial_binding"]["bound_config_keys"]
    assert "pages" in plan["initial_binding"]["bound_config_keys"]
    assert plan["refine_binding"]["transport"] == "chat_message"
...
@pytest.mark.anyio
async def test_preview_studio_card_execution_returns_bound_courseware_payload(
    app, _as_user
):
    client = TestClient(app)

    response = client.post(
        "/api/v1/generate/studio-cards/courseware_ppt/execution-preview",
        json={
            "project_id": "p-001",
            "rag_source_ids": ["file-2"],
            "config": {
                "template": "gaia",
                "pages": 16,
                "audience": "high",
    ...
    assert preview["initial_request"]["payload"]["output_type"] == "ppt"
    assert preview["initial_request"]["payload"]["options"]["template"] == "gaia"
    assert preview["initial_request"]["payload"]["options"]["pages"] == 16
    assert preview["initial_request"]["payload"]["options"]["audience"] == "high"
    assert (
        preview["initial_request"]["payload"]["options"]["include_animations"] is True
    )
    assert preview["initial_request"]["payload"]["options"]["rag_source_ids"] == [
        "file-2"
    ]
...
async def test_preview_studio_card_execution_returns_bound_word_payload(app, _as_user):
    ...
    assert (
        preview["initial_request"]["payload"]["content"]["source_artifact_id"]
        == "a-ppt-001"
    )
...
async def test_execute_studio_card_reuses_bound_courseware_session(app, _as_user):
    ...
    with (
        ...
        response = client.post(
            "/api/v1/generate/studio-cards/courseware_ppt/execute",
            json={
                "project_id": "p-001",
                "client_session_id": "client-card-ppt-001",
                "rag_source_ids": ["file-2"],
                "config": {
                    "template": "gaia",
                    "pages": 16,
...
@pytest.mark.anyio
async def test_get_studio_card_sources_returns_word_document_ppt_sources(app, _as_user):
    client = TestClient(app)
    current_artifact = SimpleNamespace(
        id="a-ppt-020",
        type="pptx",
        metadata={"title": "高中物理课件", "is_current": True},
        visibility="project-visible",
        basedOnVersionId="v-current",
        sessionId="s-020",
        updatedAt=datetime.now(timezone.utc),
...
@slow_studio_card
@pytest.mark.anyio
async def test_execute_studio_card_requires_source_artifact_for_word_document(
    app, _as_user
):
    client = TestClient(app)
...
@pytest.mark.anyio
async def test_execute_studio_card_rejects_non_ppt_source_for_word_document(
    app, _as_user
):
    client = TestClient(app)
    source_artifact = SimpleNamespace(
        id="a-summary-001", projectId="p-001", type="summary"
    )
    ...
    with (
        ...
        response = client.post(
            ...
            json={
                "project_id": "p-001",
                "source_artifact_id": "a-summary-001",
            },
        )
