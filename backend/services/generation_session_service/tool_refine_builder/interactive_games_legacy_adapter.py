"""Interactive game legacy structured refine adapter."""
...
def inject_legacy_game_html_section(html: str, section_html: str) -> str:
    if "</main>" in html:
...
async def refine_interactive_game_legacy_content(
    *,
    ...
    if is_template_game_pattern(pattern):
        ...
        if not isinstance(base_game_data, dict):
            raise ValueError(
                "interactive_games_legacy_refine_requires_structured_game_data"
            )
        ...
        updated["compatibility_zone"] = dict(LEGACY_COMPATIBILITY_ZONE)
        updated["runtime_origin"] = "legacy_compatibility"
        return updated
    ...
    updated["kind"] = "interactive_game"
    updated["html"] = inject_legacy_game_html_section(current_html, section_html)
