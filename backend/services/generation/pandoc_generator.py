...
async def call_pandoc(
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            logger.error(
                f"Pandoc failed with return code {process.returncode}: {error_msg}"
            )
            raise ToolExecutionError("Pandoc", error_msg, process.returncode)

        logger.debug("Pandoc executed successfully")
...
async def generate_docx(
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"[Task: {task_id}] Lesson plan markdown written to "
                f"{markdown_file} ({len(content.lesson_plan_markdown)} bytes)"
            )
        ...
        logger.info(
            f"[Task: {task_id}] DOCX generated successfully: {output_file} "
            f"({file_size} bytes)"
        )
        ...
        logger.error(
            f"[Task: {task_id}] Unexpected error during DOCX generation: {str(e)}",
            exc_info=True,
