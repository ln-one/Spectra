...
export function HeroSection({ onShowVideo }: { onShowVideo: () => void }) {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      <LightRays count={10} color="rgba(120, 119, 198, 0.15)" blur={40} speed={18} length="50vh" />
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/5 rounded-full blur-3xl" />

      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: prefersReducedMotion ? 0 : 0.6 }} className="mb-6">
          <Badge variant="secondary" className="gap-2 px-4 py-1.5 text-sm shadow-sm">
        ...
        >
          用 AI 重新定义
          <br />
          <WordRotate words={rotateWords} duration={2000} className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-600" />
        </motion.h1>
        ...
        >
          ...
          <Button size="lg" variant="outline" className="h-12 px-8 text-base" onClick={onShowVideo}>
        ...
        <motion.div
          ...
          className="mt-16"
        >
          <p className="text-sm text-muted-foreground mb-4">受到全国 1000+ 学校的信赖</p>
          <div className="flex flex-wrap justify-center items-center gap-8 opacity-60">
            {["清华大学附属中学", "北京大学附属中学", "中国人民大学附中", "北京师范大学附中"].map((school) => (
              <span key={school} className="text-sm font-medium text-muted-foreground">{school}</span>
            ))}
          </div>
        </motion.div>
      </div>
