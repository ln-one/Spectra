...
const PPT_WORKFLOW_STEPS = [
  {
    id: 1,
    title: "配置需求",
    description: "明确主题、页数与大纲偏好",
  },
  {
    id: 2,
    title: "共创大纲",
    description: "补充结构细节并确认生成方向",
  },
  {
    id: 3,
    title: "生成预览",
    description: "进入实时生成页查看并导出成果",
  },
] as const;
...
export function PptWorkflowRail({
  ...
  return (
    <aside
      className={cn(
        "rounded-3xl border border-zinc-200/80 bg-[linear-gradient(160deg,#ffffff,#f8fafc)] p-4 backdrop-blur-sm",
        className
      )}
      aria-label="PPT 生成流程"
    >
          ...
          {PPT_WORKFLOW_STEPS.map((step, index) => {
            ...
            return (
              <li key={step.id} className="relative pl-9">
                ...
                <span
                  className={cn(
                    "absolute left-0 top-0 flex h-[26px] w-[26px] items-center justify-center rounded-full border text-[11px] font-semibold transition-colors",
                    isCompleted
                ...
                <div className="flex items-center gap-1.5 pt-0.5">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        type="button"
                        className="inline-flex h-5 w-5 items-center justify-center rounded-full text-zinc-400 transition-colors hover:bg-zinc-100 hover:text-zinc-700"
