    ...
    <header className="h-14 border-b bg-background/80 backdrop-blur-md px-4 flex items-center justify-between shrink-0">
      <div className="flex items-center gap-2 min-w-0">
        ...
        {isEditingTitle ? (
          ...
          <button className="text-sm font-medium hover:text-primary transition-colors truncate" onClick={() => onSetEditingTitle(true)}>
            {projectTitle}
          </button>
        )}

        {activeSessionId ? (
          <span className="text-xs text-muted-foreground hidden md:inline">会话: {activeSessionId.slice(0, 8)}</span>
        ) : null}
      </div>

      <div className="flex items-center gap-2">
        <div className="hidden md:flex items-center gap-2 text-xs px-2.5 py-1 rounded-full border bg-muted/40">
          <span className={cn("w-1.5 h-1.5 rounded-full", isSessionGenerating ? "bg-amber-500 animate-pulse" : "bg-emerald-500")} />
        ...
        <Button variant="outline" size="sm" className="hidden sm:flex rounded-full h-9" onClick={onExport} disabled={!activeSessionId || isExporting}>
          <Download className="w-4 h-4 mr-2" />{isExporting ? "导出中" : "导出"}
        </Button>
        <Button variant="outline" size="sm" className="hidden sm:flex rounded-full h-9">
        ...
        <Button size="sm" className="rounded-full h-9 bg-foreground text-background hover:bg-foreground/90">
          <Play className="w-4 h-4 mr-2 fill-current" />演示
        </Button>
      </div>
    </header>
  );
